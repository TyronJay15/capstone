...
const AdminDashboard = () => {
  ...
  const setIncomingStatus = async (id, next) => {
    try {
      await updateAdminStatus(id, next);
      refreshFromStore();
      showFlash('success', `Admin approval saved: ${next}.`);
    } catch (err) {
      showFlash('error', err.message || 'Unable to save admin approval.');
    }
  };

  const toggleParentConsent = async (lrn, current) => {
    try {
      await setParentConsent(lrn, !current, 'Admin');
      refreshFromStore();
      showFlash('success', `Parent consent ${!current ? 'granted' : 'revoked'} for LRN ${lrn}.`);
    } catch (err) {
      showFlash('error', err.message || 'Unable to update parent consent.');
    }
  };
