...
import { fetchStudentGradesForTeacher } from '../services/academicsApi';
...
import { fetchTeacherRoster } from '../services/teacherApi';
import GradeEncodePanel from './teacher/GradeEncodePanel';
...
const TeacherDashboard = () => {
  ...
  const refreshRoster = useCallback(async () => {
    if (getApiBaseUrl() && localStorage.getItem('accessToken')) {
      try {
        const roster = await fetchTeacherRoster(academicYear);
        setStudents(roster);
        return;
      } catch {
        /* fallback */
    ...
    setStudents(getTeacherRoster(academicYear));
  }, [academicYear]);

  useEffect(() => {
    refreshEnrollmentStore(academicYear).then(refreshRoster);
    return subscribeEnrollmentStore(() => {
      refreshRoster();
    });
  }, [refreshRoster, academicYear]);
  ...
  const handleViewGrades = async (lrn) => {
    setGradesError('');
    if (getApiBaseUrl() && localStorage.getItem('accessToken')) {
      try {
        const result = await fetchStudentGradesForTeacher(lrn);
        if (result.blocked) {
        ...
        setGradesView({ blocked: true, student: null, reason: err.message });
    ...
    <div className="teacher-dashboard-page">
      ...
      <aside className={`teacher-sidebar ${navOpen ? 'is-open' : ''}`}>
        ...
        <nav className="teacher-side-nav">
          {['Dashboard', 'My Students', 'Student Grades', 'Encode Grades', 'Sections', 'Notes'].map((tab) => (
            <button
              key={tab}
              type="button"
      ...
      <main className="teacher-main">
        <div className="teacher-main-inner">
          ...
          {activeTab === 'Encode Grades' ? (
            <div className="teacher-table-card">
              <div className="teacher-table-header">
                <div>
                  <h2 className="teacher-table-title">Encode Grades</h2>
                  <p className="teacher-table-subtitle">
                    Save grades for your assigned subjects. Requires parent consent to view encoded records.
                  </p>
                </div>
              </div>
              {encodeMessage ? (
                <div className="teacher-encode-flash">{encodeMessage}</div>
              ) : null}
              <GradeEncodePanel
                student={selectedStudent}
                onSaved={() => {
                  setEncodeMessage('Grade saved successfully.');
                  refreshRoster();
                  window.setTimeout(() => setEncodeMessage(''), 3500);
                }}
                onError={(msg) => setEncodeMessage(msg)}
              />
              <div className="teacher-encode-roster-hint">
                Select a student from My Students or Notes, then encode here.
              </div>
            </div>
          ) : null}
