...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      ...
      if (!storedStudent && !getApiBaseUrl()) {
        navigate('/login');
        return;
      }
      ...
      if (getApiBaseUrl() && localStorage.getItem('accessToken')) {
        try {
          const dashboard = await refreshStudentSession();
          if (!cancelled && dashboard) {
            setCurrentStudent(dashboard);
            return;
          }
        } catch {
      ...
      if (storedStudent) {
        if (!cancelled) setCurrentStudent(JSON.parse(storedStudent));
      } else {
        navigate('/login');
      }
    }
  ...
  const saveProfileChanges = async () => {
    try {
      // Prepare profile data for API (convert camelCase to snake_case)
      const profileData = {
