...
export async function verifyRecaptcha({ token, demoChecked }) {
  ...
  try {
    const url = API_BASE ? `${API_BASE}/api/auth/verify-recaptcha` : '/api/auth/verify-recaptcha';
    ...
    return {
      ok: false,
      error: 'Unable to verify security check. Ensure the API server is running (npm run server).'
    };
  }
}
